...
const randomHexColor = require('./randomHexColor.js');
const colorNameToHex = require('./colorNameToHex.js');

//initialize a random hex color variable and print it out
let colorChoice = randomHexColor();
let symbol = '#';
...
userSelectedColor = luminosityChoice + hueChoice; // concatenates arguments into a valid color name

userSelectedColorAsHex = colorNameToHex(userSelectedColor); // converts color name into hex value
...
function makeBloc(width = 30, height = 10, symbol = '#', color = '#ffffff') {
  ...
  //add conditions if height and width are too small to not have negative array length
